...
\newcommand{\versiondoi}{10.5281/zenodo.22850125}
...
V11 (HDP, 0.571), V8 (ProdLDA, 0.328) and V20 (ETM, 0.490). V13 (vector-quantised VAE) underperforms
...
and the footer gives its rank within each row (LDA 2, HDP 10, ProdLDA
4, ETM 3). No recipe dominates every backbone: LDA and ETM pick V12,
with V3 and V20 close behind; HDP picks V7 (sparse absorption tokens);
ProdLDA picks V3. Every cell uses the $Q=8$ vocabulary.}
...
Bold = per-backbone winner. Recipes sorted by 4-backbone mean. LDA,
ProdLDA and ETM use the per-recipe K of \S\ref{subsec:k-policy}, so the
three fixed-K backbones are compared at one topic count per cell; HDP
truncates at 20 topics.}
...
but \textbf{HDP 25/114 (22\%)} and \textbf{ProdLDA 53/114 (46\%)}. For
HDP, 22 of the 25 are on V1, V4, V5 and V6, the recipes for which HDP
keeps between 1.2 and 2.3 effective topics ($N_{\mathrm{eff}}$,
six-scene means of the F-2 runs); across the nineteen recipes HDP's F-7
follows that effective number (Spearman $\rho = 0.59$). ProdLDA's
degenerate cells concentrate on the recipes whose per-recipe $K$ is 3 or
4: at those counts its encoder collapses onto one topic in most scenes.
The cross-backbone mean
...
ProdLDA zero on Indian Pines), V11 0.311 ($+0.008$, four ProdLDA zeros),
V12 0.393 ($+0.023$). V8 retains cross-backbone leadership under either
...
cross-backbone-consistent recipe at mean F-7 NMI $= 0.431$; it lands top-4 under
...
($0.520$) and winning ETM ahead of V12 ($0.490$ vs $0.488$). V11 (product
quantisation, vocab $M K_s = 32$) wins HDP outright ($0.571$) but
reaches only $0.292$ under LDA, $0.329$ under ETM and $0.018$ under
ProdLDA. F-7 is bounded by $\log K / H(Y)$, and V11's four-token
documents take $K = 3$ under all three fixed-K backbones, which caps its
six-scene mean at 0.46; only HDP, which keeps up to 20 topics, exceeds
that cap, so V11's profile is a nonparametric-backbone effect rather
than a recipe that beats the others at equal topic count. The same
holds for V19 under HDP (0.530 against 0.286 under LDA and 0.261 under
ETM).
