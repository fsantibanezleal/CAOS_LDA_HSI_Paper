...
a_d=\arg\min_{a\ge 0,\;\mathbf{1}^{\top}a=1}\;\lVert x_d - E\,a\rVert_2^2,\qquad
n_d^{\mathrm{V8}}[m]=\big\lfloor s\,a_{dm}\big\rceil,
\end{equation}
where $E=[e_1,\dots,e_M]$ are the NFINDR endmembers (simplex vertices)
and $s$ a fixed integer scale; $|V_{\mathrm{V8}}|=M$. The abundance
...
are genuine abundance fractions over the convex hull.
