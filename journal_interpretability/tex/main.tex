...
\newcommand{\versiondoi}{10.5281/zenodo.22826864}
...
for recipes with three or four tokens per document. V3 and V12, whose
documents hold each of their 200 tokens once, score 0.12 and 0.16, but
their top-10 is set by the order in which tied counts are sorted, and
their F-15 moves between 0.03 and 0.18 with that order. V20 re-weights
V3's identical $(\mathrm{band},\mathrm{bin})$ alphabet and scores 0.47
to 0.67 under every tie order; its topics are also less dispersed
...
the topic-stability artefact, and F-15 only after the rule is corrected
for these effects.
\end{abstract}
...
in the order of a descending sort of the counts
(\texttt{numpy.argsort}, which leaves the order among equal counts to
...
and V17 (26 of 120) and no other recipe. Alongside F-15 we compute,
...
$|\mathcal{V}|$ (Fig.~\ref{fig:dispersion}). To measure the effect of
tied counts we re-apply the rule to the archived fits with ties broken
by band index and at random (five seeds).
...
is its set of 6--12 endmembers per scene. V15 has no F-15 value: its
archived $Q=8$ document file holds the $Q=32$ vocabulary (192 tokens;
96 on Pavia~U), so F-15 could not be computed against the $Q=8$
topics (48 tokens; 24 on Pavia~U). V15 is excluded from the F-15
analyses, including Figs.~\ref{fig:dispersion} and~\ref{fig:f15vocab}.
The 1.000 of V2, V8 and V9 is fixed by the rule, and V3 and V12 depend
on the order of tied counts (Table~\ref{tab:f15ties}).}
\label{tab:f15}
\centering
\small
\begin{tabular}{lcr}
...
V13, V19) score 0.95 to 1.00 whatever the tie order, since their whole
document enters its own top-10.

\emph{(iii) Tied counts.} V3 and V12 documents hold each of their 200
tokens once, so their top-10 is chosen by the sort's order among equal
counts, not by the document's content. With ties broken by band index
or at random, V3 moves between 0.033 and 0.180 and V12 between 0.067
and 0.173, around the archived 0.117 and 0.158, and their order
reverses.

\begin{table}[ht]
\caption{F-15 under three orders of tied document counts (six-scene
means): the archived order (a descending \texttt{numpy.argsort}), band
index, and random (mean of five seeds); outcomes over the 120 judged
documents in the archived order (aligned / misaligned / ambiguous); the
number of those documents whose non-zero counts are all equal; and the
mean document length on Indian Pines.}
\label{tab:f15ties}
\centering
\scriptsize
...
\toprule
Recipe & archived & index & random & outcomes & equal-count & tokens/doc \\
\midrule
V2  & 1.000 & 1.000 & 1.000 & 120 / 0 / 0   & 0   & 200 \\
V8  & 1.000 & 1.000 & 1.000 & 120 / 0 / 0   & 9   & 14 \\
V9  & 1.000 & 1.000 & 1.000 & 84 / 0 / 36   & 120 & 1 \\
V10 & 1.000 & 1.000 & 1.000 & 120 / 0 / 0   & 120 & 3 \\
V13 & 0.950 & 0.967 & 0.957 & 114 / 6 / 0   & 120 & 4 \\
V14 & 0.950 & 0.958 & 0.955 & 114 / 6 / 0   & 120 & 16 \\
V1  & 0.792 & 0.733 & 0.817 & 95 / 25 / 0   & 0   & 450 \\
V20 & 0.642 & 0.467 & 0.667 & 77 / 43 / 0   & 0   & 1187 \\
V12 & 0.158 & 0.067 & 0.173 & 19 / 101 / 0  & 120 & 200 \\
V3  & 0.117 & 0.033 & 0.180 & 14 / 106 / 0  & 120 & 200 \\
\bottomrule
\end{tabular}
\end{table}

The one contrast that survives every tie order is V20 against V3 and
V12 (0.47 to 0.67 against at most 0.18), at the same nominal
...
alone. Across the eighteen recipes F-15 correlates negatively with
$N_{\mathrm{eff}}$ (Spearman $\rho = -0.71$) and with $|\mathcal{V}|$
($\rho = -0.60$); both correlations carry the construction effects
...
A defensible F-15 would compare the observed overlap with the overlap
expected for matched random token lists, rank a document's tokens only
where its counts are graded (or break ties by a content-based order),
and count a recipe only where the rule can return every outcome; a
semantic embedding distance between token identifiers is a further
option. We flag these as future work and recommend reading
...
eighteen recipes (V15 excluded, Table~\ref{tab:f15}); the
Spearman coefficients in the title are computed over the same
eighteen. Point colours are categorical: green for the low-$N_{\mathrm{eff}}$
...
$0.12$, $0.16$ and $0.64$; the V3 and V12 values depend on the order of
their tied counts, and V2, V8 and V9 sit at 1.0 by construction
...
\begin{enumerate}
  ...
  \item \textbf{F-15 LLM-judge.} Only after the rule is corrected for
    its construction effects (\S\ref{sec:f15}). As defined it rewards
    small vocabularies and short documents, cannot judge one-token
    documents misaligned, and depends on the order of tied counts.
\end{enumerate}
...
governed by its construction: vocabulary size, tokens per document and
the order of tied counts (\S\ref{sec:f15}). An actual LLM oracle, given
...
including the order of tied counts that defines them.
...
(V9), near 1.0 for three- and four-token documents, and for recipes
whose documents hold every token once (V3, V12) it depends on the
order of tied counts (0.03 to 0.18). V20 stays far above V3 and V12
under every tie order and has less dispersed topics
...
dispersion. We therefore recommend reporting F-15 only after the rule
is corrected for these effects, and we caution that LLM-as-judge topic
