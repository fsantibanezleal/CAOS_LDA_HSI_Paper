...
n_{\text{classes}}))$ except for B-4 (capacity sweep). This is by
...
\begin{enumerate}
\item Move per-axis $K$-sweep from a separate axis (B-4) to a layer
that wraps every other axis: report (B-1, B-2, ..., B-12) at
$K \in \{4, 6, 8, 10, 12, 16\}$ rather than at the canonical $K$
only.
\item Replace B-12 (external baseline OA delta) with an
