...
The HIDSAG citation \texttt{hidsag\_database} in
\texttt{bibliography/refs.bib} is a \texttt{@misc} dataset-descriptor
entry that cites the database itself rather than a specific
publication, pending verification of the canonical dataset
descriptor. The dataset is referenced in the companion code
repository at
\texttt{data/derived/core/hidsag\_curated\_subset.json}; if a
published dataset descriptor exists it will replace this entry
with the verified DOI / URL before the journal submission. The
present manuscript does not rely on any specific HIDSAG publication
for its findings --- all HIDSAG numerical results are read directly
from the public derived artefacts.
