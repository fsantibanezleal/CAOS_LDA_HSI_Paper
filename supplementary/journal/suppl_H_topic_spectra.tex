...
\begin{itemize}
\item Axis F-2 (topic-word coherence) in the main paper Section IV
...
distinct topics.
\item Axis F-5 (band-mask robustness): the contrast figure makes
the band-robust / band-fragile split visible at the spectrum level
that the paired-ARI summary alone does not.
\item Axis F-7 (topic--label coupling): the per-topic-card title
