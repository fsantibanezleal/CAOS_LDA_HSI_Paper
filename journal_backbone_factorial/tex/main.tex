...
\newcommand{\versiondoi}{10.5281/zenodo.22826861}
...
/ learning-rate-$10^{-3}$ / seed-42 protocol.
...
V1..V15, V17..V20). The HDP value for V15 (0.52) was computed on the
stored V15 wordification that holds the $Q=32$ vocabulary (192 tokens;
96 on Pavia~U); every other cell uses the $Q=8$ vocabulary (V15: 48
tokens; 24 on Pavia~U). The V15 cell of Fig.~\ref{fig:backbone-factorial}
and V15's affinity in Table~\ref{tab:affinity} inherit it.}
...
under ETM, fourth under ProdLDA and, leaving out the flagged V15 cell
(eleventh with it), tenth under HDP, where the leaders are V7 (absorption triplets, 0.61) and V17 (sparse-coding
...
dark cells), and the footer gives V20's rank within each row (the HDP
rank counts the flagged V15 cell of Table~\ref{tab:f2-factorial};
without it V20 is tenth). The winner moves between recipes (V12 for LDA / ETM, V3 for ProdLDA, V7
...
is: (1) fit the backbone via its deterministic builder (LDA with the
per-recipe K of \S\ref{sec:backbones}; ProdLDA and ETM with
$K = K_{P1}$ for every recipe in these F-7 runs; HDP with its 20-topic
truncation), (2) project
...
\path{data/derived/v_sweep/backbones_f7/} (HDP, ProdLDA, ETM).
...
lower level (7th under LDA and HDP); V20 is strong on average but less
cross-backbone-consistent ($\sigma = 0.12$, 6th under HDP) and is the
unique label-aware recipe here. All three have long documents and take
$K = K_{P1}$ under every backbone, so their comparison is not affected
by the topic-count difference discussed next.

\paragraph{Family 2: apparent backbone specialists} V11 (product
quantisation) wins HDP ($0.571$) and ETM ($0.530$) but collapses to
$0.292$ under LDA and $0.088$ under ProdLDA, the widest cross-backbone
spread in the sweep. V19 (UMAP coordinates) shows a similar
HDP-favoured profile. Both recipes emit three or four tokens per
document and take $K = 3$ under LDA, which caps F-7, bounded by
$\log K / H(Y)$, at a six-scene mean of 0.46; their HDP and ETM means
exceed that cap (\S\ref{subsec:v11-mechanism}).

\paragraph{Family 3: LDA-dominant} V12, V3, V14. Top-5 under LDA
but middle of the pack under the other backbones. V12's F-7 LDA
...
finding that no recipe-backbone choice is canonical.
...
\subsection{V11: winner under HDP and ETM, weak under LDA and ProdLDA}
...
$\log K / H(Y)$. Under LDA, V11's four-token documents take $K = 3$
(\S\ref{sec:backbones}), which caps F-7 at 0.40 to 0.61 across the six
scenes (six-scene mean 0.46); V11 reaches 0.77 to 0.82 of that cap on
Botswana and Salinas. The F-7 runs of ETM use $K = K_{P1}$ (12 on four
scenes) and HDP keeps up to 20 topics, so V11's 0.571 under HDP and
0.530 under ETM are out of reach of any three-topic fit. The contrast
below therefore mixes the backbone with the topic count:
...
\begin{itemize}
  \item \textbf{Under HDP}, V11 is the best recipe ($0.571$). HDP keeps
    ...
    nineteen recipes HDP's F-7 follows the number of topics it keeps
    (Spearman $\rho = 0.59$), and the recipes on which it keeps one or
  ...
  \item \textbf{Under ETM} (embedded decoder), the topic-word
    ...
    V11 is the best recipe under ETM ($0.530$).
  ...
  \item \textbf{Under ProdLDA}, V11 collapses to $0.088$, near
...
second under HDP at 0.530, after V11 (0.571), but collapses to 0.051
under ProdLDA. The recipe--backbone choice is two-dimensional.
