...
is a candidate fifth backbone but is parked pending public code.
\end{abstract}
...
HDP      & 0.54 & 0.44 & 0.31 & 0.44 & 0.44 & 0.32 & \textbf{0.61} & 0.31 & 0.35 & 0.35 & 0.50 & 0.34 & 0.31 & 0.31 & 0.52 & 0.59 & 0.42 & 0.50 & 0.41 & V7 \\
...
close behind: all four sit near the per-backbone top under every prior. V20 is the
...
\paragraph{Family 1 — consistently strong.} V8, V20, V2. V8 is top-5
under every backbone with the smallest cross-backbone standard
deviation ($\sigma = 0.06$) and leads at mean $0.431$ (winning outright
only under ProdLDA). V20 and V2 are strong on average but less
cross-backbone-consistent (V20 $\sigma = 0.12$, ranking 6th under HDP;
V2 7th under LDA and HDP); V20 is the unique label-aware recipe here.
...
family) on Salinas); ETM also picks V11 on 3/6
...
\begin{itemize}
  \item LDVAE-T~\cite{giannetti2025ldvaet}, a spectral-unmixing VAE with
    ...
    a candidate fifth backbone but has no public code at the time of
    writing; adapting it to the wordification factorial and adding its row
