...
\newcommand{\versiondoi}{10.5281/zenodo.22826844}
...
$\text{score}$ is the fold-level macro-F1. Sampling is performed with NUTS
...
targets in place of folds. The archived fits record no convergence
diagnostic such as the rank-normalised $\widehat{R}$ of Vehtari et
al.~\cite{vehtari2021rhat}. The model fixes no reference scene, so
$\mu_m$ is identified only up to the scene offsets $\delta_s$: every
$\mu_m$ shares a common shift (about $-0.17$ from the observed mean on
the labelled block) and a common posterior spread (about 0.19), and
only differences between methods are identified. We therefore report
the observed mean macro-F1 of each method, its paired difference from
the raw-spectrum baseline over the cells (scene and fold, or subset
and target) with a 94\% bootstrap interval ($B = 5000$ resamples of
the paired differences), and the pairwise probability
...
samples, 2 chains and a target acceptance of 0.9; the archived fits
record no divergence or convergence diagnostics.
...
(b) Pairwise $P[A > B]$ from the hierarchical Bayesian model (NUTS,
1000 draws, 1000 tune, 2 chains). (c), (d) HIDSAG mineralogical subsets
...
$P[\mu_{\text{soft}} > \mu_{\text{raw}}] = 0.641$. The theta-only
...
macro-F1, mean paired difference $\Delta$ from \texttt{raw\_logistic}
with its 94\% bootstrap interval over the 30 cells, and
$P = P[\mu_m > \mu_{\text{raw}}]$ from the hierarchical model. The
model's posterior locations $\mu_m$ are not listed: they are identified
only up to the scene offsets (\S\ref{subsec:b1}). All pairwise
...
\begin{tabular}{@{}lcccc@{}}
\toprule
Method & mean & $\Delta$ & 94\% interval & $P$ \\
\midrule
\texttt{topic\_routed\_soft}  & 0.915 & $+0.005$ & [$+0.003$, $+0.007$] & 0.641 \\
\texttt{raw\_logistic}        & 0.910 & ref.     & --                   & --    \\
\texttt{topic\_routed\_hard}  & 0.909 & $-0.001$ & [$-0.008$, $+0.006$] & 0.464 \\
\texttt{pca\_K\_logistic}     & 0.847 & $-0.064$ & [$-0.099$, $-0.031$] & 0.000 \\
\texttt{theta\_logistic}      & 0.586 & $-0.325$ & [$-0.359$, $-0.289$] & 0.000 \\
\bottomrule
\end{tabular}
\end{table}
...
Method & mean & $\Delta$ & 94\% interval & $P$ \\
\midrule
\texttt{raw\_logistic}           & 0.699 & ref.     & --                   & --    \\
\texttt{pca\_logistic}           & 0.674 & $-0.025$ & [$-0.050$, $-0.003$] & 0.284 \\
\texttt{topic\_logistic}         & 0.430 & $-0.268$ & [$-0.374$, $-0.176$] & 0.000 \\
\texttt{region\_topic\_logistic} & 0.392 & $-0.307$ & [$-0.409$, $-0.212$] & 0.000 \\
\texttt{cube\_topic\_logistic}   & 0.360 & $-0.339$ & [$-0.437$, $-0.246$] & 0.000 \\
\bottomrule
\end{tabular}
\end{table}
