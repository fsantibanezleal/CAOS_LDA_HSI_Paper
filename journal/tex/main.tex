...
across $K \in \{4, \ldots, 24\}$, (F-5) band-mask robustness, (F-6)
...
\caption{F-5 paired ARI heatmap across (scene, mask) tuples. KSC and
...
\begin{table}[t]
\caption{F-7 topic--label coupling: per-topic KL mean and max on the
...
``best-case band-robustness''). F-7: KL mean(canonical topics, uniform).}
\label{tab:summary}
\centering
\small
...
2026 submission focused on the F-5 band-mask diagnostic alone, also
