% ---------------------------------------------------------------------
% CAOS_LDA_HSI journal manuscript — IEEE TGRS submission
%
...
% Class  : IEEEtran (journal option, two columns, 10pt)
% Length : ~20-25 pages (within 10-page OPC-free budget where possible,
%          extending into the OPC zone for the deep evaluation tables)
% ---------------------------------------------------------------------
...
\author{%
  ...
  \thanks{Manuscript prepared 2026 for IEEE Transactions on Geoscience
    and Remote Sensing. All code, derived artefacts, and the
    interactive web application are released under MIT at
    \url{https://github.com/fsantibanezleal/CAOS_LDA_HSI}.}%
}

\markboth{IEEE Transactions on Geoscience and Remote Sensing,~Vol.~XX,
No.~Y, Month~2026}{Santib\'a\~nez-Leal: Multi-Axis Topic-Model
Evaluation on HSI}
...
same set of benchmarks: (B-1) classification accuracy under a hierarchical
Bayesian model over five method families, (B-2) topic--word coherence
($c_v$, $c_{\text{NPMI}}$, U-Mass), (B-3) seed stability for both LDA
and two neural variants (ProdLDA, ETM), (B-4) capacity sensitivity
across $K \in \{4, \ldots, 24\}$, (B-5) band-mask robustness, (B-6)
cross-method clustering agreement (LDA vs Felzenszwalb, SLIC, patches,
pixels), (B-7) topic--label coupling via $P(L \mid t)$ entropy and KL,
(B-8) per-topic Hungarian-aligned identity tracking, (B-9) HIDSAG
cross-preprocessing stability, (B-10) cross-scene topic transfer, (B-11)
rate--distortion of the topic representation, and (B-12) external
...
1726 deterministic derived artefacts (421~MB), the builder source,
...
\begin{enumerate}
\item We define twelve evaluation axes (B-1 through B-12) covering
...
model on HIDSAG.
\item We release 1726 deterministic derived artefacts (421~MB), the
...
methodological details of each axis (B-1 through B-12).
...
\midrule
B-1  & Classification (Bayesian) & method posterior $\mu_m$ & \texttt{build\_bayesian\_*} & \texttt{method\_statistics\_labelled/} \\
B-2  & Coherence                 & $c_v$, $c_{\text{NPMI}}$, U-Mass & \texttt{build\_coherence}      & \texttt{cross\_method\_agreement/} \\
B-3  & Seed stability            & mean$\pm$std ARI over 5 seeds   & \texttt{build\_neural\_topic\_seed\_stability} & \texttt{neural\_topic\_seed\_stability/} \\
B-4  & Capacity sensitivity      & ARI($K$) curve   & \texttt{build\_lda\_sweep}     & \texttt{lda\_sweep/} \\
B-5  & Band-mask robustness      & paired ARI       & \texttt{build\_band\_mask\_canonical\_comparison} & \texttt{band\_masks/canonical\_comparison.json} \\
B-6  & Cross-method agreement    & ARI matrix       & \texttt{build\_cross\_method\_agreement} & \texttt{cross\_method\_agreement/} \\
B-7  & Topic--label coupling     & H($L\mid t$), KL  & \texttt{build\_topic\_to\_data}        & \texttt{topic\_to\_data/} \\
B-8  & Per-topic identity        & Hungarian $\sigma^*$ & \texttt{build\_band\_mask\_canonical\_comparison} & same as B-5 \\
B-9  & Preprocessing stability   & ARI across recipes & \texttt{build\_hidsag\_cross\_preprocessing\_stability} & \texttt{hidsag\_cross\_preprocessing\_stability/} \\
B-10 & Cross-scene transfer      & transfer ARI     & \texttt{build\_cross\_scene\_transfer} & \texttt{cross\_scene\_transfer/} \\
B-11 & Rate--distortion          & MSE($\theta$) vs $K$ & \texttt{build\_rate\_distortion} & \texttt{rate\_distortion\_curve/} \\
B-12 & External baselines        & literature OA gap & \texttt{build\_external\_validation} & \texttt{external\_validation/} \\
\bottomrule
\end{tabular}
\end{table*}
...
\subsection{B-1: hierarchical Bayesian classification}
\label{subsec:b1}
...
\subsection{B-2: topic--word coherence}
...
\subsection{B-4: capacity sensitivity}

We sweep $K$ over the range $\{4, 6, 8, 10, 12, 16, 20, 24\}$ with the
canonical hyperparameters and report ARI(canonical fit, KMeans on
$\theta$ at $K$) against ground-truth label. The curve diagnoses
...
\subsection{B-5: band-mask robustness}
...
\subsection{B-6: cross-method agreement}
...
\subsection{B-7: topic--label coupling}
...
\subsection{B-8: per-topic identity tracking under masks}
...
\subsection{B-9: HIDSAG cross-preprocessing stability}
...
\subsection{B-10: cross-scene topic transfer}
...
\subsection{B-11: rate--distortion of $\theta$}
...
\subsection{B-12: external baseline comparison}
...
The Hyperspectral Industrial Database for the Study of Atmospheric and
Geological matter (HIDSAG \cite{escalante2018hidsag}) is a curated
collection of point-spectrum measurements over mineralogical samples,
acquired with a SWIR-only sensor. Five thematic subsets are exposed
in the companion code repository: GEOMET (general metals), MINERAL1
(silicates), MINERAL2 (carbonates), GEOCHEM (geochemical groups), and
...
Bayesian classifier of B-1 uses NUTS with $1000$ draws, $1000$ tune
...
\subsection{B-1: classification under a hierarchical Bayesian model}
\label{subsec:res-b1}

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{bayesian-method-comparison.pdf}
\caption{B-1 method posteriors under a hierarchical Bayesian model
...
\begin{table}[t]
\caption{B-1 method posteriors on the labelled-scene block (six scenes
...
\begin{table}[t]
\caption{B-1 method posteriors on the HIDSAG block (five subsets ×
...
\subsection{B-2 + B-3: coherence and seed stability of LDA, ProdLDA, ETM}

Table~\ref{tab:b3-seed} reports the seed-stability statistics on
Indian Pines for the three methods at $K = 12$, $n_{\text{seeds}} = 5$.

\begin{table}[t]
\caption{B-3 seed stability on Indian Pines, $K=12$, $n_{\text{seeds}}=5$.
...
\caption{B-5 paired ARI heatmap across (scene, mask) tuples. KSC and
Botswana paired ARI sits at $\approx 0.01$ across all four masks
(band-fragile regime); Salinas-A under SWIR-only reaches $0.766$
(band-robust regime). The remaining tuples fall in the partial-robustness
range $0.35$--$0.54$. Pavia~U $\times$ SWIR is auto-skipped (sensor
is VNIR-only).}
...
\caption{B-8 Hungarian topic-id alignment on Salinas-A under SWIR-only
...
\begin{table*}[t]
\caption{B-5 paired ARI per (scene, mask) tuple. Bold cells indicate
...
\subsection{B-6: cross-method clustering agreement}
...
\begin{table*}[t]
\caption{B-6 cross-method ARI matrix on Indian Pines. Diagonal is
...
\subsection{B-7 + B-8: topic--label coupling and per-topic identity}

We use the canonical KL divergence between $P(L \mid t)$ (the
empirical distribution of ground-truth labels conditioned on topic
$t$) and $P(L)$ (the marginal) as a per-topic concentration measure;
the within-scene mean and max are reported in
Table~\ref{tab:b7-coupling}, drawn from the
\texttt{band\_masks/canonical\_comparison.json} artefact (the
canonical-vs-masked-KL is reported on the same scale; here we use
the canonical-vs-uniform variant to express raw concentration).

\begin{table}[t]
\caption{B-7 topic--label coupling: per-topic KL mean and max on the
canonical fit of each labelled scene. Higher values denote more
sharply label-aligned topics; very high max values (KSC $\approx 27.6$
nats) reflect topic-label cells with near-zero shared support, a
signature of an ill-identified topic.}
...
Pavia University & 4.09  & 11.53 \\
Kennedy Space C. & 7.85  & 27.63 \\
Botswana         & 10.48 & 27.63 \\
\bottomrule
\end{tabular}
\end{table}

\subsection{Combining B-5 and B-1: band-fragile or label-recoverable?}
...
axes B-1, B-2, B-3, B-5, B-7 for the labelled scenes. We argue this
...
axes. B-1 column: pooled posterior $\mu_m$ for
\texttt{topic\_routed\_soft} (the canonical topic-based classifier).
B-2: $c_v$ coherence of the canonical fit. B-3: ARI mean across 5
seeds of LDA. B-5: max paired ARI across the four masks (the
``best-case band-robustness''). B-7: KL mean(canonical topics, uniform).}
\label{tab:summary}
\centering
\small
\begin{tabular}{lccccc}
\toprule
Scene & B-1 ($\mu_{\text{soft}}$) & B-2 ($c_v$) & B-3 (ARI 5-seed) & B-5 (max paired) & B-7 (KL nats) \\
\midrule
Indian Pines     & 0.741 (pooled) & 0.450 & 0.287    & 0.527 & 4.53 \\
Salinas          & 0.741 (pooled) & ---   & ---      & 0.540 & 7.11 \\
Salinas-A        & 0.741 (pooled) & ---   & ---      & \textbf{0.766} & 4.79 \\
Pavia University & 0.741 (pooled) & ---   & ---      & 0.415 & 4.09 \\
Kennedy Space C. & 0.741 (pooled) & ---   & ---      & 0.013 & 7.85 \\
Botswana         & 0.741 (pooled) & ---   & ---      & 0.127 & 10.48 \\
\bottomrule
\end{tabular}
\end{table*}

The $\mu_{\text{soft}}$ column is constant across scenes because the
B-1 model pools over scenes (the per-scene effect $\delta_s$ is a
...
the $K$-sweep on B-4 but other axes are computed at the canonical $K$.
...
B-12's external-baseline comparison uses literature OA numbers under
...
\item \textbf{Derived artefacts.} 1726 JSON and binary files,
421~MB total, including: per-scene canonical fits
...
B-5 numbers above), the cross-method agreement matrices
...
\item \textbf{Companion conference paper.} The 4--5 page WHISPERS
2026 submission focused on the B-5 band-mask diagnostic alone, also
...
inverting the labelled-scene picture. The full 1726-artefact
...
within-mask Fisher complement to B-5 that does not depend on the
