% ---------------------------------------------------------------------
% CAOS_LDA_HSI conference paper (preprint)
%
% Title  : A Band-Mask Robustness Diagnostic for Latent Dirichlet
%          Allocation on Hyperspectral Imagery
% Class  : IEEEtran conference option (typesetting only; the class does
%          not imply a venue)
% ---------------------------------------------------------------------
...
\newcommand{\versiondoi}{10.5281/zenodo.22823287}
\newcommand{\conceptdoi}{10.5281/zenodo.21504109}
...
\author{%
  \IEEEauthorblockN{Felipe Santib\'a\~nez-Leal~\orcidlink{0000-0002-0150-3246}}
  \IEEEauthorblockA{Independent Researcher\\
  Santiago, Chile\\
  ORCID: 0000-0002-0150-3246\\
  Email: \texttt{fsantibanezleal@ug.uchile.cl}}%
}

% IEEEtran's conference option locks out \IEEEaftertitletext (it would silently drop the
% header block); \IEEEoverridecommandlockouts is the class's own switch to re-enable it.
...
and five HIDSAG mineral subsets, the diagnostic separates the
scenes: topic identities on Salinas-A under SWIR-only restriction
recover the canonical assignment with paired ARI~$=0.77$, while
KSC paired ARI is~$\approx 0.01$ under every mask, and Botswana falls
to $\approx 0.01$ only under SWIR (elsewhere $0.11$--$0.13$).
The diagnostic therefore separates scenes on which any
``interpretable spectral mixture'' claim is band-robust from scenes
on which it is not. We release the full sweep (44 LDA refit
attempts, 43 successful)
...
as a route to \emph{interpretable} spectral mixtures: each spectral
unit (canonically a pixel, but optionally a region, patch, or full
sample) becomes a document of quantised band-frequency tokens, and the
...
\cite{zou2017pmlda,bioucasdias2012unmixing}.
...
benchmarks and five HIDSAG mineral subsets, totalling 44 refit
attempts (43 successful; Pavia U $\times$ SWIR is VNIR-only and
auto-skipped).
The sweep reveals a sharp split between scenes whose canonical
topics are band-robust (paired ARI 0.35--0.77) and scenes whose
topics are band-fragile (paired ARI~$\approx 0.01$).

\item A \textbf{public, deterministic artefact set} (3895 JSON and
...
into a discrete band-frequency document using the V1 recipe
introduced in~\cite{santibanezleal2022procemin}: each spectrum is
...
(uses the unmasked spectrum's Fisher ranking; a clean
information-theoretic ablation is out of scope of the diagnostic).
\end{itemize}
...
For every (scene, mask) tuple we compute:
...
Section~\ref{sec:results} reports (i) and (ii); (iii) and (iv) are
recorded in the per-tuple artefacts described in
...
benchmarks. KSC paired ARI is $\approx 0.01$ across all four masks
(Botswana only under SWIR; $0.11$--$0.13$ under the other three);
Salinas-A under SWIR-only reaches $0.77$.}
...
$0.013$ across all four masks: the masked LDA assigns
pixels to topics with at most chance-level agreement with the
canonical fit. Botswana is at chance only under SWIR ($0.013$); under
VNIR ($0.108$), no-water ($0.127$) and \mbox{top-50} Fisher ($0.117$) it sits
at $0.11$--$0.13$, still below the band-robust scenes but not
chance-level. On KSC the canonical-fit topic identity is \emph{not}
recoverable from any reasonable band subset.
...
(KSC swap rates 0.53--0.64; Salinas-A swir swap rate 0.18; the full
table is in the public artefact set described in
Section~\ref{sec:repro}).
...
(paired ARI $=0.77$). Five of six canonical topics have a dominant
masked topic with confusion-matrix share $> 0.65$; the sixth ($c_0$)
splits across two masked topics (38 and 28 pixels). Most of the
residual swap rate comes from $c_1$ (120 of the 232 unmatched pixels):
its dominant masked topic is also the dominant topic of $c_4$, and the
one-to-one Hungarian assignment gives it to $c_4$.}
...
is degenerate and the mask falls back to keep-all-bands; the fallback
is recorded in the artefact metadata. Paired-ARI comparison
...
with the location of the cellulose and lignin absorption features in
the short-wave infrared~\cite{curran1989foliar}.

On KSC under every mask, and on Botswana under the SWIR mask, the
diagnostic returns a partial-negative result. Paired
ARI~$\approx 0.01$ means a masked-LDA observer would
...
issue: KSC has 13 classes sampled to 2,686 documents of 5,211 labelled
pixels; Botswana has 14 classes sampled to 2,775 of 3,248). The diagnostic does not
...
The full sweep is released as 24 (labelled scene, mask) tuples plus 20
...
Hungarian alignment), the per-pixel \path{dominant_topic_map.bin}
(uint8 $H \times W$ with sentinel 255 for unlabelled), and the
per-pixel \path{theta_grid.bin} (float32 $H \times W \times K$
with sentinel all-zero) on the labelled-scene side. Total derived
footprint: 3895 deterministic artefacts, 449~MB.

{\raggedright
A FastAPI backend serves the artefacts at three endpoints:
\path{/api/band-masks}, \path|/api/band-masks/{scene}/{mask}|,
\path{/api/band-masks/canonical-comparison}, plus the HIDSAG
companions. A test harness covering 109 endpoints verifies endpoint
health and the content of the single-page-application shell at each
deployment.\par}
...
band-fragile (KSC under every mask, Botswana under SWIR, paired ARI
$\approx 0.01$). The full sweep, including 3895 deterministic JSON and binary artefacts,
...
release~\cite{hidsag2023figshare}. The present paper is
methodologically distinct: it proposes a band-mask robustness
diagnostic for LDA on HSI and applies it, among other benchmarks,
to the HIDSAG mineralogical subsets. HIDSAG is cited as a data
source, not as a methodological contribution of this work. The
author declares no other conflict of interest.
